...
input_folder_path = 'resource/asnlib/publicdata'   #sys.argv[1]
val_file = 'resource/asnlib/publicdata/yelp_val.csv'        #sys.argv[2]
output_file = 'Project.csv'        #sys.argv[3]
...
def get_users_features(record):
    ...
    if (d['user_id'] and d['user_id']!='') and (d['review_count'] and d['review_count']!='' and d['fans']!='' and d['useful']!='' and d['average_stars']!=''):     #users_review_count,fans,useful,average_stars
        mytuple = (int(d['review_count']),int(d['fans']),int(d['useful']),float(d['average_stars']),int(d["funny"]),int(d["cool"]),int(d["compliment_cool"]),int(d["compliment_funny"]),int(d["compliment_list"]),int(d["compliment_note"]), int(d["compliment_more"]), int(d["compliment_photos"]),int(d["compliment_profile"]), int(d["compliment_writer"]))
